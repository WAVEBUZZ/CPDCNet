...
class SKConv(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1,M=2,r=16,L=32):
        ...
        d=max(in_channels//r,L)   # 计算向量Z的长度d
        self.M=M
        self.out_channels=out_channels
        self.conv=nn.ModuleList()  # 根据分支数量添加不同核的卷积操作
        for i in range(M):
            # 为提高效率，扩张卷积5x5为 3X3，dilation=2）来代替。 且论文中建议组卷积G=32
        ...
        self.global_pool=nn.AdaptiveAvgPool2d(1)  # 自适应pool到指定维度，这里指定为1，实现GAP
        self.fc1=nn.Sequential(nn.Conv2d(out_channels,d,1,bias=False),
                               nn.BatchNorm2d(d),
                               nn.ReLU(inplace=True))   # 降维
        self.fc2=nn.Conv2d(d,out_channels*M,1,1,bias=False)  # 升维
        self.softmax=nn.Softmax(dim=1)  # 指定dim=1，使得两个全连接层对应位置进行softmax,保证对应位置a+b+..=1
...
class SKBlock(nn.Module):
    ...
    '''
    expansion = 2  # 指每个block中通道数增大指定倍数
...
class SKNet(nn.Module):
    ...
    def __init__(self, nums_class=1000, block=SKBlock, nums_block_list=[3, 4, 6, 3]):
        ...
        self.conv=nn.Sequential(nn.Conv2d(64, 64, 7, 2, 3, bias=False),
                                nn.BatchNorm2d(64),
                                nn.ReLU(inplace=True))
        self.maxpool = nn.MaxPool2d(3, 2, 1) # kernel=3x3 stride=2 padding=1
        self.layer1 = self._make_layer(block, 128, nums_block_list[0], stride=1)  # 构建表中每个[]的部分
        ...
        self.layer4 = self._make_layer(block, 32, nums_block_list[3], stride=2)
    ...
    def forward(self, input):
        # print(input.shape)
        output=self.conv(input)  # B*64*64*64
        # print(f"第一个7*7卷积：{output.shape}")

        output=self.maxpool(output) # B*64*32*32
        # print(f"第一个最大池化：{output.shape}")
        output=self.layer1(output)
        # print(f"第一残差块：{output.shape}") # B*256*32*32
        output=self.layer2(output)
        # print(f"第二残差块：{output.shape}") # B*512*16*16
        output=self.layer3(output)
        # print(f"第三残差块：{output.shape}") # B*1024*8*8

        # print('test 第三个残差块')
        # print(output.shape)
        output=self.layer4(output)
        # print(f"第四残差块：{output.shape}") #第四残差块：torch.Size([16, 2048, 4, 4])
        output = self.avgpool(output)
        # print(f"平均池化：{output.shape}") #平均池化：torch.Size([16, 2048, 1, 1])
        # output=output.squeeze(-1).squeeze(-1)
        # print(f'去掉1维度的的shape{output.shape}')
        # output=self.fc(output)
        # output=self.softmax(output)
        # print(f"全连接加softmax之后：{output.shape}")  #全连接加softmax之后：torch.Size([16, 64])
        return output
