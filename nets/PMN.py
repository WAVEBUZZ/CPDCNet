...
class PMN(nn.Module):
    ...
    def forward(self, x):
        ...
        out_org = out_org.unsqueeze(-1)  # 调整out_org的形状为[2, 176, 1]
        # print(f"out_org:{out_org.shape}")

        self.outlayer = nn.Linear(1, 1584, 16)  # 调整outlayer的形状为[1, 1584, 16]
        # #进行全连接
...
def main():
    ...
    tmp1 = torch.randn(16, 682, 25, 25)
    # tmp2 = torch.randn(128, 560, 25, 25)
    # tmp3 = torch.randn(128, 1302, 25, 25)
    out1 = net.forward(tmp1)[0]
    out2 = net.forward(tmp1)[1]
