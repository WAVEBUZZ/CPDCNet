...
def VGG16(pretrained, in_channels = 3, model_ft=None,  **kwargs):
    model = VGG(make_layers(cfgs["D"], batch_norm = False, in_channels = in_channels), num_classes=2 ,**kwargs)


    if pretrained:
        model.load_state_dict({k.replace('module.', ''): v for k, v in torch.load("model_data/vgg16-397923af.pth" ).items()})

        model.load_state_dict(checkpoint['state_dict'], strict=False)
        # state_dict = load_state_dict_from_url(r"D:\unet-pytorch-main\model_data\vgg16-397923af.pth", model_dir="./model_data")
    ...
    #         state_dict[k] = v.reshape(num_classes, 4096, 1, 1)
        # pre_weights = load_state_dict_from_url("https://download.pytorch.org/models/vgg16-397923af.pth",
